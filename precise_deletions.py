#!/usr/bin/env python3
"""
Precise deletions following the exact instructions from οδηγίες.txt
"""
...
def apply_precise_deletions(articles):
    """Apply precise deletions exactly as specified in οδηγίες.txt"""
    
    changes_made = []
    
    # Define exact deletion texts from οδηγίες.txt
    deletion_texts = {
...
def main():
    print("=== PRECISE DELETIONS FOLLOWING ΟΔΗΓΙΕΣ.TXT ===\n")
    ...
    modified_articles, changes_made = apply_precise_deletions(articles)
    
    # Save changes
    save_articles(modified_articles)
    
    # Report
    print(f"\n=== PRECISE DELETION REPORT ===")
    ...
    # Save report
    report = {
        'precise_deletions': changes_made,
        'total_deletions': len(changes_made),
        'timestamp': '2025-02-27T07:05:00Z'
    }
    
    with open('precise_deletions_report.json', 'w', encoding='utf-8') as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    
    print(f"\nPrecise deletions report saved to precise_deletions_report.json")
