#!/usr/bin/env python3
"""
Script to apply specific text deletions to articles and delete 2 articles completely
"""
...
def apply_deletions(articles):
    """Apply the specific deletions to articles"""
    
    # Define the deletions for each article
    deletions = {
...
*\xa0 \xa0Η Τεχνολογία Βιοσυντονισμού του\xa0 «Sensitiv Imago»\xa0\xa0προσφέρει την δυνατότητα\xa0να έχουμε πλήρη εικόνα της υγείας, των διατροφικών μας ελλείψεων και της κατάστασης του μεταβολισμού μας και εντοπίζει διαταραχές στο αρχικό στάδιο ακόμη και πριν εκδηλωθούν. \xa0
\xa0\xa0 \xa0 Η μέθοδος είναι εξαιρετικά οικονομική, μη επεμβατική, ανώδυνη, ακίνδυνη ακόμη και σε παιδιά και ο χρόνος που απαιτείται είναι μόνον 60 – 90 λεπτά της ώρας.

\xa0  Περισσότερες πληροφορίες για την μέθοδο θα βρείτε στο τμήμα της σελίδας του\xa0blog\xa0που αναφέρεται αποκλειστικά στο\xa0«Sensitiv Imago».\xa0"""
        ],
...
Μετά από 3 μήνες είναι καλό να επαναλάβετε τον έλεγχο ώστε να βεβαιωθείτε για την πρόοδό σας και να προσαρμοστεί το πρόγραμμα Διατροφής και Συμπληρωμάτων Διατροφής, αναλόγως."""
        ],
        "Φρούτα, τα οφέλη και οι αντιθέσεις τους": [
            """Κατερίνα Μυστριώτη:\xa0Περισσότερα σχετικά με τους συνδυασμούς και τα οφέλη των τροφών για μια διατροφή υγιεινή που να σου χαρίζει Ενέργεια και Δύναμη επικοινώνησε μαζί μου στο τηλ.\xa06975 301223\xa010πμ-19μμ ή στείλε μου e-mail:\xa0mistrioti@gmail.com
...
\xa0\xa0\xa0\xa0\xa0\xa0\xa0 Άρθρο που δημοσιεύτηκε στο terra papers\xa0
\xa0\xa0\xa0 Ετικέτες\xa0Diet Coaching"""
        ],
...
Κατερίνα Μηστριώτη\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0(σύμβουλος κλινικής διατροφολογίας)
...
"\xa0\xa0\xa0\xa0Λεπτίνη, η ορμόνη ρυθμιστής της Ενέργειας" άρθρο που δημοσιεύτηκε στο\xa0terra papers"""
    ...
    for article in articles:
        ...
        # Check if article should be deleted completely
        ...
        # Apply specific deletions
        if title in deletions:
            for deletion_text in deletions[title]:
                # Remove the text (handle multiple occurrences)
...
def main():
    ...
    # Apply deletions
    modified_articles, changes_made = apply_deletions(articles)
    ...
    report = {
        ...
        'timestamp': '2025-02-27T06:25:00Z'
    }
    
    with open('deletion_report.json', 'w', encoding='utf-8') as f:
